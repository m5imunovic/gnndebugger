...
class GATv2MultiDiGraphNet(nn.Module):
    def __init__(
        self,
        num_layers: int,
        node_features: int,
...
class GATv2MultiHeadBlockUnidirect(nn.Module):
...
class GATv2MultiHeadBlockBidirect(nn.Module):
    def __init__(self, hidden_features: int, heads: int, concat: bool, layer_norm: bool = True):
        super().__init__()
        self.concat = concat
        self.layer_norm = layer_norm

        self.gat_fw = GATv2Conv(
            in_channels=hidden_features,
            out_channels=hidden_features,
            heads=heads,
            concat=concat,
    ...
    def forward(self, h, edge_attr, edge_index):
        ...
        if self.concat:
            h = self.lin(h)
        if self.layer_norm:
            h = self.ln(h)  # TODO: Do we want LayerNorm with GATv2?
        return h
